...
function LoginPage({ onLogin }) {
  ...
  return (
    <div style={{
      ...
      }}>Sovereign AI Lab · Restricted · Authorised Access Only</div>
      {/* Bottom bar */}
      ...
      <div style={{
        ...
        <div style={{
          ...
          transition:"border-color 0.3s", padding:"32px 28px", position:"relative",
        }}>
          {/* Corner accents */}
          ...
          <div style={{textAlign:"center",marginBottom:28}}>
            ...
            <div style={{
              ...
              color:glitch?C.accent:C.p1,textTransform:"uppercase",
          ...
          {/* Demo hint — clickable rows pre-fill both fields */}
          <div style={{
            padding:"7px 10px",marginBottom:14,
            background:C.bg2,border:`1px solid ${C.line2}`,
            fontSize:"8px",color:C.p3,lineHeight:1.7,fontFamily:mono,
          }}>
          ...
          {isSuccess && (
            <div style={{padding:"8px 12px",marginBottom:14,background:C.greenDim,
          ...
          {/* Scan steps */}
          {isScanning && (
            <div style={{marginBottom:14,padding:"10px 12px",background:C.bg0,border:`1px solid ${C.line2}`}}>
          ...
          <div style={{marginTop:18,textAlign:"center"}}>
            ...
            <div style={{display:"flex",justifyContent:"center",gap:10}}>
              {["DEFAULT-DENY","AUDIT-LOGGED","SESSION-BOUND"].map(tag=>(
                <span key={tag} style={{fontSize:"7.5px",letterSpacing:1,color:C.p3,
                  padding:"1px 6px",border:`1px solid ${C.line}`,textTransform:"uppercase"}}>{tag}</span>
              ))}
            </div>
          </div>
        </div>
...
// Module-level flag set by GovernorDashboard UI toggle.
// When true, evaluate() returns block immediately — proving
// Stage D: "failure reduces capability, not increases it."
...
function evaluate(tool, args, ctx, extraPolicies=[], killSwitch=false, sessionHistory=[]) {
  ...
  if (_DEGRADED_MODE) {
    return {
      ...
      trace: [{ layer:0, key:"kill", outcome:"block", risk:100,
      ...
      trustTier: "internal", piiHits: [], confidenceGap: false,
    };
  }
  ...
  let risk=trust.floor, decision="allow", matched=[];
...
const SEED_DATA = [
  // Benign ops only — seeds dashboard with realistic baseline without triggering auto-KS.
  // Attacks and blocks are demonstrated live via Action Tester and Agent Simulator.
  { tool:"http_request",   args:{url:"http://localhost/api/health"},              ctx:{agent_id:"claude-ops-agent",   trust_level:"internal"}, agent:"claude-ops-agent" },
  { tool:"http_request",   args:{url:"http://localhost/api/metrics"},             ctx:{agent_id:"crewai-monitor-v1",  trust_level:"internal"}, agent:"crewai-monitor-v1" },
  { tool:"messaging_send", args:{to:["ops@acme.com"],content:"Status: all systems nominal"}, ctx:{agent_id:"claude-ops-agent", trust_level:"internal"}, agent:"claude-ops-agent" },
  { tool:"file_read",      args:{path:"/var/log/app/access.log"},                ctx:{agent_id:"langchain-pipeline", trust_level:"internal"}, agent:"langchain-pipeline" },
  ...
  return (
    <div>
      ...
      <div style={{display:"flex", flexWrap:"wrap", gap:12, marginTop:8, paddingTop:8,
        ...
        <span style={{color:C.muted}}>LAYERS: <span style={{color:C.text}}>{trace.length}/6</span></span>
...
function AuditTrailTab({ auditLog, policySnapshots }) {
  ...
  const exportJSON = () => {
    ...
    a.href = url; a.download = `ocg-audit-${Date.now()}.json`; a.click();
    ...
    <div style={{display:"flex", flexDirection:"column", height:"100%", background:C.bg0}}>
        ...
        <div style={{display:"flex", alignItems:"center",
          ...
          <div style={{display:"flex", gap:8, alignItems:"center"}}>
            ...
            <button onClick={exportJSON} style={{
              fontFamily:mono, fontSize:8, letterSpacing:1, padding:"3px 10px",
              border:`1px solid ${C.accent}`, color:C.accent,
      ...
      <div style={{flex:1, overflow:"auto"}}>
        {auditLog.length === 0 ? (
          ...
          <div key={e.id}>
          <div onClick={()=>setExpanded(prev=>prev===i?null:i)}
            style={{
            display:"grid", gridTemplateColumns:"120px 110px 1fr 80px 90px",
            gap:0, padding:"8px 16px",
            borderBottom:`1px solid ${C.line}`,
            background:expanded===i?C.bg2:i%2===0?C.bg0:C.bg1,
          ...
          {/* Expanded detail row */}
          {expanded===i && (
            <div style={{padding:"10px 16px 12px", background:C.bg2,
              borderBottom:`1px solid ${C.line}`, borderLeft:`3px solid ${C.accent}`}}>
              <div style={{fontFamily:mono, fontSize:8, color:C.p3, letterSpacing:1,
                textTransform:"uppercase", marginBottom:6}}>FULL DETAIL</div>
              {e.expl && (
                <div style={{fontFamily:sans, fontSize:11, color:C.p1,
                  lineHeight:1.6, marginBottom:4}}>
                  <span style={{fontFamily:mono, fontSize:8, color:C.p3}}>EXPLANATION: </span>
                  {e.expl}
                </div>
              )}
              {e.tool && (
                <div style={{fontFamily:mono, fontSize:9, color:C.p2, marginBottom:2}}>
                  TOOL: <span style={{color:C.p1}}>{e.tool}</span>
                  {e.policy && e.policy !== "none" && (
                    <> · POLICY: <span style={{color:C.amber}}>{e.policy}</span></>
                  )}
                </div>
              )}
              {e.chainAlert && (
                <div style={{fontFamily:mono, fontSize:9, color:C.red, marginBottom:2}}>
                  ⛓ CHAIN ESCALATION: {e.chainAlert}
                </div>
              )}
              {e.pii && (
                <div style={{fontFamily:mono, fontSize:9, color:C.amber, marginBottom:2}}>
                  🏷 PII DETECTED in payload
                </div>
              )}
              <div style={{fontFamily:mono, fontSize:8, color:C.p3, marginTop:4}}>
                EVENT ID: {e.id} · SEQ: {e.seq}
              </div>
            </div>
          )}
          </div>
...
// ─── DEMO: in-memory user management (no backend) ───────────
let _nextId = 5;
function genKey() {
...
function AdminUserManagementTab() {
  ...
  const revoke = (id) => {
    ...
    updateUser(id, {is_active:false, api_key:null}); setPending(null);
  };
  const restore  = (id) => updateUser(id, {is_active:true, api_key:genKey()});
  const rotateKey = (id) => {
    updateUser(id, {api_key:genKey()}); setRotated(id);
  ...
  return (
    <div style={{padding:20}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
        ...
        <div>
          <div style={{fontFamily:mono,fontSize:13,fontWeight:700,color:C.p1,letterSpacing:1}}>User Management</div>
          <div style={{fontFamily:mono,fontSize:9,color:C.p3,marginTop:3,letterSpacing:1}}>
            {active.length} active · {inactive.length} revoked · admin access only · <span style={{color:C.amber}}>demo mode — in-memory</span>
          </div>
        </div>
        <button onClick={()=>setSC(s=>!s)} style={{fontFamily:mono,fontSize:9,letterSpacing:1,padding:"6px 14px",
      ...
      {showCreate&&(
        <div style={{padding:16,marginBottom:20,background:C.bg2,border:`1px solid ${C.accent}`,borderTop:`2px solid ${C.accent}`}}>
            ...
            {[{label:"Name",key:"name",type:"text",ph:"Jane Smith"},{label:"Email",key:"email",type:"email",ph:"jane@org.io"},{label:"Password",key:"password",type:"password",ph:"••••••••"}].map(({label,key,type,ph})=>(
              <div key={key} style={{display:"flex",flexDirection:"column",gap:4}}>
                ...
                <input type={type} value={form[key]} placeholder={ph} onChange={e=>setForm(f=>({...f,[key]:e.target.value}))}
                  style={{background:C.bg0,border:`1px solid ${C.line2}`,color:C.p1,fontFamily:mono,fontSize:10,padding:"7px 10px",outline:"none",width:"100%",boxSizing:"border-box"}}/>
              </div>
            ))}
            <div style={{display:"flex",flexDirection:"column",gap:4}}>
              <label style={{fontFamily:mono,fontSize:8,letterSpacing:2,color:C.p3,textTransform:"uppercase"}}>Role</label>
              <select value={form.role} onChange={e=>setForm(f=>({...f,role:e.target.value}))}
                style={{background:C.bg0,border:`1px solid ${C.line2}`,color:C.p1,fontFamily:mono,fontSize:10,padding:"7px 8px",outline:"none"}}>
          ...
          <button onClick={create} disabled={creating} style={{fontFamily:mono,fontSize:9,letterSpacing:1,padding:"7px 18px",
            border:`1px solid ${C.green}`,color:C.green,background:C.greenDim,cursor:creating?"not-allowed":"pointer",opacity:creating?0.5:1}}>
      ...
      {active.map(u=>(
        <div key={`${u.id}-${u.api_key}`} style={{display:"grid",gridTemplateColumns:"1fr 200px 90px 80px 220px",
          ...
          <div style={{display:"flex",gap:4}}>
            <button onClick={()=>rotateKey(u.id)} style={{fontFamily:mono,fontSize:8,padding:"3px 8px",cursor:"pointer",
              border:`1px solid ${rotated===u.id?C.green:C.line2}`,color:rotated===u.id?C.green:C.p3,
              background:rotated===u.id?C.greenDim:"transparent",transition:"all 0.2s"}}>
              {rotated===u.id?"✓ ROTATED":"↻ KEY"}
            </button>
            <button onClick={()=>revoke(u.id)} style={{fontFamily:mono,fontSize:8,padding:"3px 8px",cursor:"pointer",
              border:`1px solid ${pending===u.id?C.red:C.line2}`,color:pending===u.id?C.red:C.p3,
      ...
      {inactive.length>0&&(
        <>
          <div style={{fontFamily:mono,fontSize:8,color:C.p3,letterSpacing:2,textTransform:"uppercase",padding:"12px 12px 4px"}}>REVOKED ACCOUNTS</div>
          {inactive.map(u=>(
      ...
      {users.length===0&&<div style={{fontFamily:mono,fontSize:9,color:C.p3,textAlign:"center",padding:"24px 0"}}>No users. Create the first operator above.</div>}
    </div>
  );
}
...
const ROLE_TABS = {
  admin:    ["dashboard","tester","simulator","policies","audit","topology","users"],
  operator: ["dashboard","tester","simulator","policies","audit","topology"],
  auditor:  ["dashboard","audit","topology"],
};
...
function GovernorDashboard({ userRole="operator", userName="", onLogout=()=>{} }) {
  const [tab, setTab]       = useState("dashboard");
  const [killSwitch, setKS]       = useState(false);
  const [degraded,   setDegraded] = useState(false);
  const [autoKsEnabled, setAutoKs] = useState(false); // off by default — manual control
  ...
  // If 3+ blocks in the last 10 evaluated actions OR avg risk ≥ 82
  // → autonomously engage kill switch without human instruction
  const AUTO_KS_BLOCK_THRESHOLD = 3;
  const AUTO_KS_RISK_THRESHOLD  = 82;
  // autoKsCooldown: set to true when operator manually resumes.
  // Prevents auto-KS from immediately re-engaging after a manual resume.
  // Cleared automatically once threat conditions genuinely drop below both thresholds.
  const autoKsCooldown = useRef(false);

  // Wrapped resume handler — sets cooldown so auto-KS backs off after manual resume
  const handleKSResume = () => {
    autoKsCooldown.current = true;
    handleKS(false);
  };

  useEffect(()=>{
    if (!autoKsEnabled) return;  // auto-KS disabled — manual control only
    if (killSwitch) return;      // already engaged — nothing to do
    ...
    if (autoKsCooldown.current) {
      if (!threatActive) autoKsCooldown.current = false;
      return;
    }
  ...
  const handleDegraded = val => {
    ...
    addAudit("DEGRADED_MODE", { action: val ? "ENGAGED" : "RELEASED",
      label: val ? "DEGRADED MODE — control-plane failure simulated. All execution halted (default-deny)."
                 : "Degraded mode released — control plane restored to normal operation." });
    if (val) showNarr("🔴 DEGRADED MODE ACTIVE — Control-plane failure simulated. evaluate() returns BLOCK on every call. Proving Stage D: failure reduces capability, not increases it.");
    else     showNarr("✅ Control plane restored. Normal 5-layer evaluation resumed.", 4000);
  };
      ...
      <div style={{display:"flex", alignItems:"center", justifyContent:"space-between",
        ...
        <div style={{display:"flex", alignItems:"center", gap:10}}>
          ...
          {userRole==="admin" || userRole==="operator" ? (
            <div style={{display:"flex", gap:4}}>
              <button onClick={()=>handleKS(true)} disabled={killSwitch} style={{
                fontFamily:mono, fontSize:9, letterSpacing:1.5, padding:"5px 12px",
                border:`1.5px solid ${killSwitch?C.line2:C.red}`,
                color:killSwitch?C.p3:C.red, background:"transparent",
                cursor:killSwitch?"default":"pointer", opacity:killSwitch?0.4:1,
                transition:"all 0.12s"}}>
                ☠ KILL
              </button>
              <button onClick={handleKSResume} disabled={!killSwitch} style={{
                fontFamily:mono, fontSize:9, letterSpacing:1.5, padding:"5px 12px",
                border:`1.5px solid ${!killSwitch?C.line2:C.green}`,
                color:!killSwitch?C.p3:C.green, background:"transparent",
                cursor:!killSwitch?"default":"pointer", opacity:!killSwitch?0.4:1,
                transition:"all 0.12s"}}>
                ✓ RESUME
              </button>
      ...
      <div style={{flex:1, display:"grid", gridTemplateColumns:"1fr 268px",
        ...
        <div style={{background:C.bg0, overflow:"auto", display:"flex", flexDirection:"column", gap:1}}>
          ...
          <div style={{background:C.bg2, flexShrink:0, padding:14}}>
            ...
            <div style={{display:"flex", alignItems:"center", justifyContent:"space-between",
              marginBottom:10, padding:"6px 8px", background:C.bg0,
              ...
              <div>
                ...
                <div style={{fontFamily:mono, fontSize:9, fontWeight:600,
                  ...
                  {autoKsEnabled?"ON — autonomous":"OFF — manual only"}
                </div>
              </div>
              <button onClick={()=>setAutoKs(v=>!v)} style={{
                ...
                border:`1px solid ${autoKsEnabled?C.amber:C.line2}`,
                color:autoKsEnabled?C.amber:C.p3,
                background:autoKsEnabled?"rgba(245,158,11,0.1)":"transparent",
                cursor:"pointer", transition:"all 0.15s"}}>
                {autoKsEnabled?"DISABLE":"ENABLE"}
              </button>
            </div>
            ...
            <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10}}>
              ...
              <div style={{display:"flex", gap:4}}>
                ...
                <Btn onClick={handleKSResume} style={{fontSize:8, padding:"4px 9px"}}>RESUME</Btn>
              </div>
            </div>
            {/* Degraded mode — Req 4 Default-Deny / Stage D proof */}
            <div style={{marginTop:10, paddingTop:10, borderTop:`1px solid ${C.line}`}}>
              <div style={{fontFamily:mono, fontSize:8, color:C.muted, letterSpacing:1.5,
                textTransform:"uppercase", marginBottom:4}}>DEGRADED MODE
                <span style={{color:C.p3, marginLeft:6, fontSize:7}}>Stage D proof</span>
              </div>
              <div style={{display:"flex", alignItems:"center", justifyContent:"space-between"}}>
                <div style={{fontFamily:mono, fontSize:11, fontWeight:700,
                  color:degraded?C.red:C.p3}}>
                  {degraded?"SIMULATING":"NORMAL"}
                </div>
                <div style={{display:"flex", gap:4}}>
                  <Btn onClick={()=>handleDegraded(true)}  variant="red" style={{fontSize:8, padding:"4px 9px"}}>DEGRADE</Btn>
                  <Btn onClick={()=>handleDegraded(false)} style={{fontSize:8, padding:"4px 9px"}}>RESTORE</Btn>
                </div>
              </div>
              {degraded && (
                <div style={{marginTop:6, fontFamily:mono, fontSize:8, color:C.red,
                  padding:"5px 8px", border:`1px solid ${C.red}`, background:C.redDim,
                  lineHeight:1.5}}>
                  ⚠ Control-plane failure active — all evaluate() calls return BLOCK.
                  Proving: failure reduces capability, not increases it.
                </div>
              )}
            </div>
...
export default function App() {
  ...
  const handleLogin = (userData) => {
    ...
    setTimeout(() => { setUser(userData); setTransitioning(false); }, 400);
  };
  const handleLogout = () => {
    setTransitioning(true);
    setTimeout(() => { setUser(null); setTransitioning(false); }, 300);
  };

  if (transitioning) return (
    <div style={{minHeight:"100vh",background:C.bg0,display:"flex",alignItems:"center",
      justifyContent:"center",fontFamily:"'IBM Plex Mono',monospace",fontSize:10,
      color:C.p3,letterSpacing:3,textTransform:"uppercase"}}>
      {user ? "SECURING SESSION…" : "INITIALISING GOVERNOR…"}
    </div>
  );

  if (!user) return <LoginPage onLogin={handleLogin}/>;
  return <GovernorDashboard userRole={user.role} userName={user.name} onLogout={handleLogout}/>;
}
