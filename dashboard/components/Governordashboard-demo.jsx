...
// ═══════════════════════════════════════════════════════════
// TAB: API KEYS — self-service key management (demo mode)
// ═══════════════════════════════════════════════════════════
function ApiKeysTab() {
  const [apiKey, setApiKey]     = useState("ocg_xK92mNpR44aB1cDeFgHiJkLmNoPqRsTuVwXyZ");
  ...
  const rotateKey = () => {
    ...
    const key = "ocg_" + Array.from({length:36}, ()=>chars[Math.floor(Math.random()*chars.length)]).join("");
    setApiKey(key);
    setShowFull(true);
    setCopied(false);
  ...
  return (
    <div style={{padding:20, maxWidth:720}}>
      <div style={{marginBottom:24, paddingBottom:14, borderBottom:`1px solid ${C.line2}`}}>
        ...
        <div style={{fontFamily:mono,fontSize:8,color:C.amber,marginTop:6,letterSpacing:0.5}}>
          ● DEMO MODE — keys shown are simulated
        </div>
      </div>
        ...
        <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16}}>
          <div>
            ...
            <div style={{fontFamily:mono,fontSize:9,color:C.p2}}>
              Owner: <span style={{color:C.p1, fontWeight:600}}>demo-user</span>
            </div>
          </div>
      ...
      <div style={{background:C.bg1, border:`1px solid ${C.line}`, padding:20, marginBottom:20}}>
        ...
        <button onClick={rotateKey} style={{
          ...
          {confirmRotate?"CONFIRM REGENERATE":"↻ REGENERATE API KEY"}
        </button>
      </div>
...
client = GovernorClient(
    base_url="https://openclaw-governor-demo.fly.dev",
...
{`curl https://openclaw-governor-demo.fly.dev/evaluate \\
...
const ALL_TABS = [
  ...
  { id:"simulator", label:"Agent Simulator",   icon:"⚡" },
...
function SurgeTab({ receipts, stakedPolicies, setStaked, userRole }) {
  ...
  const surgeDim   = "rgba(139,92,246,0.08)";
...
// ═══════════════════════════════════════════════════════════
// REDESIGNED GOVERNOR DASHBOARD — left sidebar, wider content
// ═══════════════════════════════════════════════════════════
function GovernorDashboard({ userRole="operator", userName="", onLogout=()=>{} }) {
  ...
  const [narr, setNarr]     = useState("🛡 OpenClaw Governor v0.3.0 ACTIVE — clean session. Use Agent Simulator to run the demo sequence.");
  ...
  const addAudit = (type, data) => {
    setAuditLog(prev => [{
      id: `${Date.now()}-${Math.random().toString(36).slice(2,6)}`,
      type, ts: new Date().toISOString(), time: new Date().toLocaleTimeString(),
      ...data,
    }, ...prev].slice(0, 500));
  };
  ...
  const onResult = useCallback((tool, r, agent) => {
    setSessionMemory(prev => {
      ...
      return { ...prev, [agent]: [...hist, { tool, policy:r.policy, ts:Date.now() }].slice(-50) };
    });
    addAudit("DECISION", {
      tool, agent, decision: r.decision, risk: r.risk, policy: r.policy,
      expl: r.expl,
...
export default function App({ onExit }) {
  ...
  return <GovernorDashboard key={user.username} userRole={user.role} userName={user.name} onLogout={handleLogout}/>;
}
